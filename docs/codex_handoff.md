...
3. Supported Telegram commands:
   ...
   - `/ask <question>`: prompt-based operator assistant (position summary, market lookup, deliberation recommendation)
   - `/market <name>`: market-focused alias of `/ask` for ticker/name lookup against local scan logs
...
10. Prompt agent:
   - `python3 scripts/step51_operator_agent.py --prompt "current position and similar markets for Philadelphia spread"`

`step44` does not place orders. It is read-only and answers `/status`, `/state`, `/queue`, `/health`, `/teams`, `/progress`, `/nexttrade`, `/digest`, `/cycle`, `/brain`, `/performance`, `/refresh`, `/refreshstatus`, `/refreshreset`, `/ask`, and `/market` using step43–step51 snapshots/logs.
