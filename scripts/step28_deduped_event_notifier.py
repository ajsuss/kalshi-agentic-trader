...
DEDUP_PATH = "logs/step28/notified_keys.jsonl"
...
def format_step26(record: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            f"Confirm flag: {d.get('confirm_live_send')}",
        ]
    )
...
def dedupe_key(event_name: str, message: str) -> str:
...
def record_seen_key(path: str, key: str, event_name: str, message: str) -> None:
    append_jsonl(
        path,
        {
            "ts": utc_now_iso(),
...
def main() -> None:
    ...
    log_path = "logs/step28/deduped_event_notifier.jsonl"
    ...
    key = dedupe_key(args.event, message)
    seen = load_seen_keys(DEDUP_PATH)

    skipped_duplicate = key in seen and not args.force
    ...
    if not skipped_duplicate:
        ...
        record_seen_key(DEDUP_PATH, key, args.event, message)
        ...
        "checks": {
            ...
            "telegram_send": {
                ...
                "details": {
                    ...
                    "telegram_ok": telegram_payload.get("ok") if telegram_payload else None,
                },
            },
        },
