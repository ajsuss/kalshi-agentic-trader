...
STEP51_PATH = REPO_ROOT / "scripts" / "step51_operator_agent.py"
...
def run_operator_agent_prompt(prompt: str) -> dict:
    completed = subprocess.run(
        ["python3", str(STEP51_PATH), "--prompt", prompt],
        capture_output=True,
        text=True,
        cwd=str(REPO_ROOT),
        timeout=60,
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"step51 operator agent failed with code {completed.returncode}: {completed.stderr.strip()}"
        )
    return json.loads(completed.stdout)
...
def format_operator_agent_response(payload: dict) -> str:
    text = payload.get("response_text", "")
    actions = payload.get("actions", [])
    source = payload.get("snapshot_source")
    suffix = f"\n\n(actions={actions}, snapshot_source={source})"
...
def handle_updates(base_url: str, configured_chat_id: str | None, updates: list[dict]) -> int | None:
    ...
    for update in updates:
        ...
        if normalized not in {"/status", "/state", "/health", "/queue", "/teams", "/progress", "/nexttrade", "/digest", "/cycle", "/brain", "/performance", "/refresh", "/refreshstatus", "/refreshreset", "/ask", "/market"}:
            continue
        ...
        try:
            ...
            elif normalized in {"/ask", "/market"}:
                if not command_arg:
                    response_text = "Usage: /ask <question> (or /market <name>)"
                else:
                    response_text = format_operator_agent_response(run_operator_agent_prompt(command_arg))
            else:
                team_snapshot = load_team_status_json()
